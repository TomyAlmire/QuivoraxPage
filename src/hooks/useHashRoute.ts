...
function parseHash(hash: string): { branch: BranchId | null; node: string | null; works?: boolean } {
  // formatos: #/  ·  #/sistemas  ·  #/web/web-frontend  ·  #/trabajemos  ·  #/trabajos
  const path = hash.replace(/^#\/?/, '').split('/').filter(Boolean);
  if (path.length === 0) return { branch: null, node: null };

  const [a, b] = path;
  if (a === 'trabajos') return { branch: null, node: null, works: true };
  ...
  if (NODE_BY_ID[a]) {
    ...
    if (n.kind === 'root') return { branch: null, node: null, works: true };
...
export function useHashRoute(): void {
  ...
  useEffect(() => {
    return useMapStore.subscribe((s) => {
      ...
      const next = routeToHash(s.branch, s.node, s.mode === 'works');
